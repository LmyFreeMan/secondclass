...
//        var_dump($actinfo);
	...
	public function cheakuser()
	{
        ...
        $data=DB::name('activeuserinfo')->where('schoolnum',$username)->where('passwd',$password)->find();
        if($data)
        {
	...
	public function quit()
	{
		...
		$this->success('退出成功');
		return $this->redirect("index/index/index.html");
	}
	public function  edit(){
	    if(count($_POST)>0)
        {
          ...
          $exit=Db::table('activeuserinfo')->where('passwd',$_POST['passwod'])->where('schoolnum',$_POST['schoolnum'])->find();
            if($exit)
            {
                Db::table('activeuserinfo')->where('schoolnum',$_POST['schoolnum'])->setField('passwd', $_POST['passwordnew']);
                $this->success("修改成功");
            }
            else{
                $this->error("账号/密码错误");
